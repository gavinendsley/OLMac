...
defaultConfigFile = "/Library/Preferences/com.apple.securityproxy_mail.plist"
...
defaultConfig = {
    "service" : [
                    ...
                    {   "type"                  : "imap",
                        "enabled"               : True,
                        "dest_address"          : "", # address or host
                        "proxy_ssl_enabled"     : True,
                        "dest_ssl_enabled"      : False,
                        "bind_port"             : 993,
                        "dest_port"             : 143,

                        "ssl_certificate_path"  : "", # Public key
                        "ssl_private_key_path"  : "", # Private key

                        "auth_to_server"        :   {
                                                       "crammd5" : True ,   # crammd5
                                                       "login"   : True ,   # base64 
                                                       "plain"   : True ,   # base64 
                                                       "clear"   : True ,   # clear text
                                                    }
                    },  
...
class Config (object):
    ...
    def Set(self, pathStr, valueStr):
        ...
        if type(destValue) == aBoolType:
            if type(valueStr) == aBoolType:
                boolValue = str(valueStr)
            else:
                boolValue = valueStr

            aStrType = type("")
            if type(valueStr) == aStrType: 
                boolValue = valueStr.lower()            
...
# testing code
    ...
    try: 
        prefsConfig = Config(defaultConfig)
