...
class TwistedLog:
    ...
    def Open(facility):
        ...
        syslog.startLogging(facility, 0)
    

class ProxyLog:

 syslogEnabled = True
 facilityName = "com.apple.securityproxy_mail"
 ...
 smtp = "SMTP"
 proxy = "Proxy"
 imapAccessTag = "V1:IMAP:ACCESS["
 smtpAccessTag = "V1:SMTP:ACCESS["
 proxyAccessTag = "V1:PROXY:ACCESS["
 accessLevel = 1
 ...
 def Open(facility):
    ...
    TwistedLog.Open(facility)
    syslog.openlog(facility, syslog.LOG_PID)

 @staticmethod
 def SetAccessTagVersion(level):
    ProxyLog.accessLevel = level
    ProxyLog.imapAccessTag = "V%d:IMAP:ACCESS[" % level
    ProxyLog.smtpAccessTag = "V%d:SMTP:ACCESS[" % level
    ProxyLog.proxyAccessTag ="V%d:PROXY:ACCESS[" % level
 ...
 @staticmethod
 def LogAccIMAP( *args):
    ProxyLog.AccMsg([ProxyLog.imapAccessTag], args)
 ...
 @staticmethod
 def LogAccSMTP( *args):
    ProxyLog.AccMsg([ProxyLog.smtpAccessTag], args)
 ...
 @staticmethod
 def Acc( *args):
    ProxyLog.AccMsg([ProxyLog.proxyAccessTag], args)
        

 @staticmethod
 def Err( *args):
    strlist = ["V1:PROXY:ERR["]
    ProxyLog.ErrMsg(strlist, args)

 
 @staticmethod
 def Info( *args):
    strlist = ["V1:PROXY:INFO["]
    ProxyLog.ErrMsg(strlist, args)
 ...
 def Debug( *args):
    ...
    strlist = ["V1:PROXY:DEBUG["]
    ProxyLog.ErrMsg(strlist, args)

class ProcessClass:
    pidFilePath = "/var/run/com.apple.securityproxy_mail.pid"
    
    @staticmethod
    def removePIDFile():
        try:
            os.remove(ProcessClass.pidFilePath)
        except:
            ProxyLog.Err("Error deleting PID file:")
             
    @staticmethod
    def writePID():
        open(ProcessClass.pidFilePath,'wb').write(str(os.getpid()))
            
    
    @staticmethod
    def removePID():
          if os.path.exists(ProcessClass.pidFilePath):
            try:
                file = None
                file = open(ProcessClass.pidFilePath)
                pid = int(file.read())                
            except:
                pass
            ...
            try:
                ...
                if why[0] == errno.ESRCH:
                    ProxyLog.Debug( "Removing previous pidfile: ", ProcessClass.pidFilePath, " pid: ", pid)
            ProcessClass.removePIDFile()
...
class EnvironmentClass:
    ...
    def GetNumProcessors():
        try:
            result = Popen(["sysctl", "hw.logicalcpu"], stdout=PIPE).communicate()[0]
            #expected result is "hw.logicalcpu: 2\n"
            resultArray = result.split(":", 2)
            numSplits = len(resultArray)
            if numSplits != 2:
                raise
                           
            numProcessors = resultArray[1].strip(" \t\r\n")
            result = int(numProcessors)
            if result < EnvironmentClass.MinProcessors: 
                raise
                
            if result > EnvironmentClass.MaxProcessors:
                result = EnvironmentClass.MaxProcessors
                
            return result
        
        except: 
            return EnvironmentClass.MinProcessors
...
# testing code
    ...
    print "EnvironmentClass.GetNumProcessors() = " , EnvironmentClass.GetNumProcessors()
    
    ProxyLog.Open()
    ProxyLog.Err("test err")
    ProxyLog.Access("test access")
    ProxyLog.Err( "test ", "err =", 123)
    ProxyLog.Acc("test ", "err =", 123)
    ProxyLog.LogAccIMAP( "test ", "err =", 1)
    ProxyLog.LogErrIMAP("test ", "err =", 2)
    ProxyLog.LogAccSMTP( "test ", "err =", 3)
    ProxyLog.LogErrSMTP("test ", "err =", 4)
